...
export const getUsers = async (
  ...
  if (status === "not-pending-or-declined") {
    ...
    const uniqueUsers = Array.from(
      new Map(allRelevantUsers.map((user) => [user.id, user])).values(),
    );
    return uniqueUsers;
  }
...
export const debarUser = async (user: User): Promise<void> => {
  ...
  const blacklistRef = doc(db, "blacklist", normalizedEmail);
...
export const removeEmailFromBlacklist = async (
  ...
  const blacklistRef = doc(db, "blacklist", normalizeEmail(email));
...
export const removeEmailFromWhitelist = async (
  ...
  const whitelistRef = doc(db, "whitelist", normalizeEmail(email));
...
export const getCalendarTasksForUser = (
  ...
  Promise.all([getDocs(assignedQuery), getDocs(createdQuery)]).then(() => {
...
export const getAnnouncements = (
  ...
  return onSnapshot(q, (querySnapshot) => {
    const announcements = querySnapshot.docs.map(
...
export const getResources = (
  ...
  return onSnapshot(q, (querySnapshot) => {
    const resources = querySnapshot.docs.map(
